...
class MVTecFSCopyPasteDataset(Dataset):
    ...
    def copy_paste(self, img, mask):
        ...
        if self.in_fg_region:
            ...
            # Zoek masker in één van de submappen van ground_truth
            search_pattern = os.path.join(self.dataset_path, self.class_name, 'ground_truth', '*', f'{img_name}_mask.png')
            mask_files = glob.glob(search_pattern)

            if len(mask_files) > 0:
                fg_path = mask_files[0]
                fg_mask = Image.open(fg_path)
                fg_mask = np.asarray(fg_mask)
            else:
                # Geen masker gevonden, maak een leeg masker aan
                img_height, img_width = n_image.shape[:2]
            ...
            if (np.sum(intersect_mask) > int(2 / 3 * np.sum(aug_mask == 255))):
                # wanneer grootste deel van aug_mask in fg_mask zit
